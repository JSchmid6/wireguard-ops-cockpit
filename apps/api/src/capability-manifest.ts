...
export function capabilityPlannerContract(): string {
  return [
    ...
    "For the fixed IMSM disk container /dev/md127, a read-only status may run as /usr/local/sbin/cockpit-disk-action status with no runAsUser, no network, and empty readablePaths/writablePaths: the sandbox supplies the kernel's /proc/mdstat as a fixed read-only bind. Removals and additions are never manifest steps — the sandbox refuses them. Express them in a shell fence as the exact typed commands mdadm --manage /dev/md127 --add|--remove /dev/sdX or /usr/local/sbin/cockpit-disk-action add|remove <sdX>; the typed disk executor runs the reviewed helper and reports the actions it ran.",

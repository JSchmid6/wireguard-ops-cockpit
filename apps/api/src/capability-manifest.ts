...
export function capabilityPlannerContract(): string {
  return [
    ...
    "For a WordPress update in the fixed docroot /var/www/html, use /usr/local/sbin/cockpit-wordpress-update with status, check, verify, update, or rollback, no runAsUser, outbound network, and empty readablePaths/writablePaths. The root-owned helper takes a verified database dump and file archive before it changes anything, updates core, database schema, plugins and themes, verifies checksums afterwards, and restores both automatically on any failure — so the manifest declares risk contained, not data_loss: the loss is structurally excluded rather than merely promised. It offers no arbitrary wp command, no path argument, and no plugin install or delete. Use check first; rollback takes an optional backup stamp and otherwise restores the most recent one.",
    "For the fixed IMSM disk container /dev/md127, a read-only status may run as /usr/local/sbin/cockpit-disk-action status with no runAsUser, no network, and empty readablePaths/writablePaths: the sandbox supplies the kernel's /proc/mdstat as a fixed read-only bind. Removals, additions, and SMART actions are never manifest steps — the sandbox refuses them (it provides no block devices and drops the raw-IO capabilities SMART needs). Express them in a shell fence as the exact typed commands mdadm --manage /dev/md127 --add|--remove /dev/sdX or /usr/local/sbin/cockpit-disk-action add|remove|smart|smarttest <sdX> (whole sd[a-z] disks only); smart returns the drive's SMART JSON (health, attributes, selftest log) read-only and smarttest starts its short self-test without touching data; the typed disk executor runs the reviewed helper and reports the actions it ran.",
    "For a machine-level return path before a change that file snapshots cannot cover (package upgrades, service migrations, schema changes), use /usr/bin/python3 /usr/local/lib/wireguard-ops-cockpit/cockpit-vps-snapshot with status, list, or create <name> as an earlier step of the same manifest, no runAsUser, outbound network, and empty readablePaths/writablePaths. The root-owned helper talks to the hoster's API for exactly this instance, creates a block-level snapshot of the whole machine (the hoster keeps it 30 days), reports its id, duration, and whether the instance stayed responsive, and rotates only cockpit-owned snapshots when the plan's limit is reached. delete <snapshotId> and revert <snapshotId> always require operator approval: a revert restarts the host and discards everything written since the snapshot. The snapshot is a guaranteed return path, not an automatic rollback — name the snapshot id in the rollback section. The helper offers no other instance, no endpoint argument, and never exposes the hoster credentials.",

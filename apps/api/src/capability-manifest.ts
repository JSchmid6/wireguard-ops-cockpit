...
export function capabilityPlannerContract(): string {
  return [
    ...
    "For Nextcloud app lifecycle changes, use /usr/local/sbin/cockpit-nextcloud-app-action with php-install, php-enable, php-status, exapp-catalog-refresh, exapp-register, or exapp-status plus a syntactically valid app id, runAsUser www-data, outbound network, and empty readablePaths/writablePaths. The helper supplies its fixed internal scopes; never guess apps-external or other implementation paths. exapp-catalog-refresh resets only the regenerable AppAPI app-store catalog through Nextcloud AppData and immediately refetches it. This semantic helper intentionally offers no disable, uninstall, or arbitrary occ command. Never claim those excluded modes exist in rollback text; describe them only as a separate future reviewed recovery action.",

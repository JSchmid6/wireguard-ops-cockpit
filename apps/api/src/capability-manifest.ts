...
export function capabilityPlannerContract(): string {
  return [
    ...
    "For enabling GitLab's restricted rootless DinD service policy, use /usr/bin/python3 /usr/local/lib/wireguard-ops-cockpit/cockpit-gitlab-runner-rootless-dind enable with readablePaths and writablePaths both set exactly to /etc/gitlab-runner/config.toml, network none, and risk identity_or_secret. The helper preserves privileged=false for jobs, adds only services_privileged=true and the four reviewed docker:*dind-rootless allowlist patterns, refuses multiple runners or any conflicting policy, and never prints tokens.",

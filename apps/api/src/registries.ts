...
function buildPlannerPrompt(prompt: string): string {
  return [
    ...
    "Produce a complete runbook plan. Output the plan and EXIT.",
    "CRITICAL: Do NOT ask for approval or operator review.",
    "CRITICAL: Do NOT say 'awaiting approval' or 'checkpoint' or 'ready for review'.",
    "CRITICAL: Produce the final output and stop. No follow-up turns.",
    "You have full context of the VPS via the AGENTS.md in this repo.",
    "Rules:",
    "- No human checkpoints. Runbooks execute autonomously.",
    "- Self-recover from non-destructive errors. Stop on data loss or security issues.",
    "- The safety review agent checks every runbook before execution.",
    "Output format:",
    ...
    const modelArgs = runtimeOptions.opencodeModel
     ...
     const script = [
      ...
      'export DEEPSEEK_API_KEY="${DEEPSEEK_API_KEY:-}"',
       ...
       "  exec bash",
       "fi",
      `DEEPSEEK_API_KEY="\${DEEPSEEK_API_KEY:-}" "$planner_executable" run --auto ${modelArgs.join(" ")} --print-logs ${shellQuote(agentPrompt)}`,
      'status=$?',

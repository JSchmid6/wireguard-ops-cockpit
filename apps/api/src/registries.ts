...
function buildPlannerPrompt(prompt: string): string {
  return [
    ...
    "Your job is to produce a runbook — a shell script that runs autonomously in a tmux session.",
    "You have full context of the VPS via the AGENTS.md in this repo.",
    "CRITICAL RULES:",
    "- No human checkpoints. Runbooks execute autonomously without operator intervention.",
    "- Attempt self-recovery for non-destructive errors (retry, fallback, alternative approach).",
    "- NEVER attempt recovery if it involves: data deletion, privilege escalation, or security bypass.",
    "- If a destructive error occurs, STOP and report — do not attempt self-repair.",
    "The safety review agent checks every runbook before execution and blocks dangerous operations.",
    "Keep the output structured:",
    "1. Summary (one sentence)",
    "2. Runbook script logic",
    "3. Parameters",
    "4. Error handling",
    "5. Risk class",
    "6. Impact",
    ...
    const modelArgs = runtimeOptions.opencodeModel
     ...
     const script = [
      ...
      'export DEEPSEEK_API_KEY="${DEEPSEEK_API_KEY:-}"',
       ...
       "  exec bash",
       "fi",
      `DEEPSEEK_API_KEY="\${DEEPSEEK_API_KEY:-}" "$planner_executable" run ${modelArgs.join(" ")} --print-logs ${shellQuote(agentPrompt)}`,
      'status=$?',

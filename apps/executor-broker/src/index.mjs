...
export function validateRequest(value, now = Date.now()) {
  ...
  if (action !== "service.restart" && action !== "service.status") throw new Error("unsupported capability action");
  if (!services.has(target)) throw new Error("service target is not allowlisted");

...
The generated `AGENTS.md` also contains the root-controlled curated `context/system-architecture.md`. It carries deployment facts that cannot be safely inferred from a source tree, including the configured Nextcloud AppAPI HaRP daemon. Research must distinguish this authoritative context from live observations and explicit uncertainty. In particular, Nextcloud app configuration commonly resides in the database; absence from `config.php` or a single app directory is not evidence that a daemon is unconfigured.
...
The repeated-change circuit breaker counts failures per runtime fingerprint (planner model, safety model, capability contract, broker role contract, and semantic executor-boundary contract). A deployed and verified runtime repair therefore starts a fresh failure audit without erasing history; repeated failures under the unchanged runtime still stop autonomous retries.

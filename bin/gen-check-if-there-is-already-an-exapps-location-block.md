# Check if there is already an exapps location block in the
> Generated 2026-07-11T19:05:13.671Z

## Prompt
Check if there is already an /exapps/ location block in the nginx or apache config for nextcloud.wejos.de. Find the config file in /etc/nginx/sites-enabled/ or /etc/apache2/sites-enabled/ that contains 'nextcloud.wejos.de'. Then check if it already has a 'location /exapps/' or 'ProxyPass /exapps/' line. Output the full config file path and whether /exapps/ is already configured.

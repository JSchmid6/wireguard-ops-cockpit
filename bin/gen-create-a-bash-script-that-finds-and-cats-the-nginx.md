# Create a bash script that finds and cats the nginxapache co
> Generated 2026-07-11T19:39:47.020Z

## Prompt
Create a bash script that finds and cats the nginx/apache config for nextcloud.wejos.de. Run: ls /etc/nginx/sites-enabled/ 2>&1; ls /etc/apache2/sites-enabled/ 2>&1; for f in /etc/nginx/sites-enabled/*; do if grep -q nextcloud.wejos.de $f; then cat $f; fi; done; for f in /etc/apache2/sites-enabled/*; do if grep -q nextcloud.wejos.de $f; then cat $f; fi; done

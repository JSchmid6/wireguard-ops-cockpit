...
- The typed disk helper `cockpit-disk-action` backs `disk.manage` against the fixed IMSM container `/dev/md127`: `status` reports container members and volume redundancy; `remove`/`add` accept only whole `sd[a-z]` disks, refuse a second removal from an already degraded volume, refuse disks with a filesystem or foreign metadata, refuse while a rebuild runs, and verify every mutation against `/proc/mdstat`; `smart` returns one drive's SMART JSON (health, attributes, self-test log) read-only, and `smarttest` starts its short self-test as root without touching data. Plans may use only `mdadm --manage /dev/md127 --add|--remove /dev/sdX`, `mdadm --detail /dev/md12[67]`, or the exact `/usr/local/sbin/cockpit-disk-action status|add|remove|smart|smarttest` CLI; every other mdadm or helper form remains a shell exception. Sandboxed disk steps are read-only: the capability sandbox pins the kernel's `/proc/mdstat` read-only at `/run/mdstat` for the helper and refuses `remove`/`add` as well as `smart`/`smarttest` before execution (it has no block devices and no raw-IO capabilities) — mutations and direct device access run only through the typed executor path.
...
- Retain a generated capability by digest only after independent post-execution verification, and only when its manifest declares `"retain": true` (operations expected to recur); one-off repairs stay in the job history and audit only.
...
- Machine-level return paths use `cockpit-vps-snapshot` (Contabo API, this instance only). `status`, `list`, and `create <name>` are autonomous with outbound network; `delete` and `revert` always require operator approval because a revert restarts the host and discards everything written since the snapshot. The executor binds `/etc/wireguard-ops-cockpit/contabo.env` read-only into that step only; the whole directory is a hard policy boundary, never a plan target. A `cockpit-capability/v2` manifest may declare `{"kind":"vps"}` as a scope: the executor snapshots before the first step, treats the snapshot as a guaranteed return path, and never reverts automatically. Only `cockpit-` prefixed snapshots are rotated when the hoster's limit is reached.
- Machine self-update of the cockpit stand itself is a typed capability (`self.update`, read-only `self.status`). The only sudo-exposed entry point is `cockpit-self-update-action`, and its only accepted forms are `status`, `diff <sha>` (read-only review input), and `<sha> <diff-sha256>`: one full lowercase 40-hex commit sha merged into the allowlisted origin plus the sha256 of the diff the running Cockpit reviewed (a bare sha is refused as unreviewed). The helper starts the one-shot `wireguard-ops-cockpit-self-update@<sha>-<diff-sha256>.service` unit; `cockpit-self-update-run` behind it is deliberately not a sudo target and re-verifies the repo directory, its origin remote, the merge, and the reviewed diff hash itself before it runs the reviewed `homeserver-cockpit-deploy.sh` with `REPO_COMMIT=<sha>`. Plans keep the line `cockpit-self-update-action <sha>`; the API reviews the code first (`doc/setup/self-update.md`, Review before install) and stops for the operator only when the review flags, fails, or could not see a focus area completely. Everything else (missing/short/uppercase shas, extra arguments, a foreign deploy script or repo) is refused with a structured JSON error before anything is touched.
- A self-update never restarts cockpit services synchronously: the deploy script prepares files, writes the state, and returns with `COCKPIT_RESTART_MODE=defer`; the runner then verifies the host (services, socket files, `api_health`, and `web_url` on the WireGuard IP when present) and schedules a one-shot activation timer (`systemd-run --on-active`, default 600s). The activation restarts the four cockpit services, verifies them again, and is refused only if the deployed commit was superseded in the meantime. `/var/lib/wireguard-ops-cockpit/self-update/state.json` (root, 0750) carries `deployed_commit`, `previous_commit` (the rollback reference), `web_url`, and the activation status; each run appends to `/var/log/wireguard-ops-cockpit-self-update/history.jsonl`. Concurrent updates are serialized by a lock and refused with code 66.
- The self-update module is installed by the deploy script like every other helper (`deploy/helpers/`, `deploy/systemd/`, `deploy/sudoers/cockpit-executor`), so rolling out a stand that contains it is what enables machine self-update for every later stand. `doc/setup/self-update.md` records the exact install lines and the job flow.

...
- Machine self-update of the cockpit stand itself is a typed capability (`self.update`, read-only `self.status`). The only sudo-exposed entry point is `cockpit-self-update-action`, and its only accepted forms are one full lowercase 40-hex commit sha that must be merged into the allowlisted origin, or `status`. The helper starts the one-shot `wireguard-ops-cockpit-self-update@<sha>.service` unit; `cockpit-self-update-run` behind it is deliberately not a sudo target and re-verifies the repo directory, its origin remote, and the merge itself before it runs the reviewed `homeserver-cockpit-deploy.sh` with `REPO_COMMIT=<sha>`. Everything else (missing/short/uppercase shas, extra arguments, a foreign deploy script or repo) is refused with a structured JSON error before anything is touched.

...
For `disk.manage` the plan may contain only the exact typed forms `mdadm --manage /dev/md127 --remove /dev/sdX`, `mdadm --manage /dev/md127 --add /dev/sdX`, `mdadm --detail /dev/md12[67]`, or the same operations as `/usr/local/sbin/cockpit-disk-action status|remove|add|smart|smarttest [sdX]`; any other mdadm or helper invocation stays blocked as a shell exception. `smart <sdX>` returns that disk's SMART JSON (health, attributes, self-test log) read-only; `smarttest <sdX>` starts its short self-test without touching data. The helper refuses a second removal while a volume is already degraded (one disk at a time) and refuses disks that carry a filesystem, foreign metadata, or arrive while a rebuild is running. A read-only status may also run as a capability-manifest step (`cockpit-disk-action status`): the sandbox pins the kernel's `/proc/mdstat` read-only at `/run/mdstat` for it; `remove`/`add`/`smart`/`smarttest` are never manifest steps — the sandbox refuses them (no block devices, no raw-IO capabilities) and they run only through the typed executor.
...
- Autonomous mutation currently supports only the typed service helper allowlist (`systemctl restart|status` on allowlisted services) and the typed disk helper (`mdadm --manage /dev/md127 --add|--remove /dev/sdX`, `mdadm --detail /dev/md12[67]`, or the equivalent `cockpit-disk-action status|add|remove|smart|smarttest` CLI, where `smart` reads a disk's SMART JSON and `smarttest` starts its short self-test); `blocked_prerequisite` means a reviewed helper must be added first. Sandboxed disk steps stay read-only (status only); mutations and SMART actions are never sandbox steps.
